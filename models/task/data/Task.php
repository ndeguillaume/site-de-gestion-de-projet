...
class Task{
    ...
    public function getRelatedIssues($id) {
        $sql = 'SELECT fk_issues_id FROM task_issues_dependency WHERE fk_task_id = '.$id.'AND project_id = '.$_SESSION["project_id"];
        return $this->db->query($sql);
    }

    public function setTaskToDo($project_id, $id){
        $sql = $this->db->prepare('INSERT INTO task_todo (project_id, fk_task_id) 
    ...
    public function setTaskDone($project_id, $id){
        $sql = $this->db->prepare('INSERT INTO task_done (project_id, fk_task_id) 
        VALUES(:project_id, :id);');
        $sql->execute([
            'project_id' => $project_id,
            'id' => $id
