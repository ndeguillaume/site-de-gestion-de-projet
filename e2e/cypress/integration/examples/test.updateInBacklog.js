context('Delete in backlog', () => {
    ...
    it('update title of an issue', () => {
        cy.task('queryDb', "INSERT INTO `issue` (project_id, order_in_sprint, title, description, priority, cost)" +
            ...
            .then(() => {
                cy.get('.issue-information .issue-information-title').click()
                    .clear()
                    .type('nouveau titre').should('have.value', 'nouveau titre')
                    .type('{enter}')
                    .then(()=>{
                        cy.get('.issue-information .btn-success').click();
                        cy.task('queryDb', "SELECT title FROM `issue` WHERE id = 1;")
                            .then( results => {
                                expect(results[0]).to.deep.equal({ title: 'nouveau titre' });
                            })
                    })
            });        
    });
    ...
    it('update title of an issue', () => {
        cy.task('queryDb', "INSERT INTO `issue` (project_id, order_in_sprint, title, description, priority, cost)" +
            ...
            .then(() => {
                cy.get('.issue-information .issue-information-description').click()
                    .clear()
                    .type('nouvelle description').should('have.value',"nouvelle description")
                    .type('{enter}')
                    .then(()=>{
                        cy.get('.issue-information .btn-success').click();
                        cy.task('queryDb', "SELECT description FROM `issue` WHERE id = 1;")
                            .then( results => {
                                expect(results[0]).to.deep.equal({ description: 'nouvelle description' });
                            })
                    })
            });        
    });
