...
class Issue
{
    ...
    {
        $sql = 'INSERT INTO issue(project_id, sprint_id, title, order_in_sprint) VALUES ('.$project_id.', '.$sprint_id.', "'.$title.'", '.$order_in_sprint.')';
    ...
    {
        $sql = 'DELETE FROM issue where project_id = '.$project_id.' AND id = '.$id;
        if ($this->db->query($sql)) {
            return true;
        }
        return false;
    ...
    {
        $sql = 'UPDATE issue SET title = "'.urldecode($title).'",description="'.urldecode($description).'",cost='.$cost.', priority="'.$priority.'" WHERE id = '.$id.' AND project_id = '.$project_id;
        if ($this->db->query($sql)) {
            return true;
        }
        return false;
