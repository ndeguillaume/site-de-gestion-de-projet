...
class Tasks
{
    ...
    public function endSprint()
    {
        ...
        $this->db->query($sql);
        $sql = 'DELETE t_d FROM task_issues_dependency AS t_d 
        INNER JOIN task_done AS t ON t_d.fk_task_id = t.fk_task_id AND t_d.project_id = t.project_id 
        WHERE t_d.project_id=' . $_SESSION["project_id"];
        $this->db->query($sql);
    ...
    public function getTasksWithoutTest() {
        $sql = "SELECT id, title FROM task WHERE project_id =".$_SESSION["project_id"]." AND id NOT IN (SELECT fk_task_id FROM test WHERE fk_task_id IS NOT NULL);";
        $this->db->query($sql);
